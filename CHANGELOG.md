...
## 0.47.0 — Pièces jointes à chaque étape, rapporteur sans délégation, étape SCC garantie et stockage GED classé

- **Élu rapporteur — liste épurée** : dans la fiche de l'acte, la liste des rapporteurs n'affiche que les **noms et prénoms** (en gras) ; la **fonction** et les **délégations** n'y figurent plus. La **délégation concernée se choisit au champ suivant** (« Délégation de l'élu rapporteur ») quand l'élu en porte plusieurs. (`frontend/src/pages/Dossier.tsx`.)
- **Pièces jointes à chaque étape** : un dossier accepte des **annexes à toutes les étapes du circuit**, et plus seulement pendant la rédaction. Le **détenteur de l'étape courante** (ou son délégué habilité) et le rédacteur peuvent **ajouter, remplacer, réordonner ou retirer** une pièce. Nouveau droit `canAttach` (`acl.js`) adossé à un crochet du moteur de circuit (`engine.js`), appliqué aux mutations d'annexes (`annexes.service.js`).
- **Étape finale SCC garantie** : un acte **rédigé par le SCC** (agent ou directrice de la DAJCP) **conserve toujours l'étape finale de validation SCC** — c'est l'étape finale pour **tous les cas de figure** (l'auto-validation et la validation implicite ne la contournent jamais). (`engine.js`.)
- **Stockage GED classé par catégorie** : les fichiers déposés dans Alfresco (ou le volume local) sont désormais rangés par **catégorie métier** — annexes (et leur PDF converti), pièces de séance, convocations, ordres du jour, cahiers, parapheur, contrôle de légalité, documents source, gabarits, logos, **reprise AIRS**, divers — avec un **nom lisible** et des **métadonnées** (titre, description : organisme, acte, séance, nature, matière, rubrique, déposant). Fin de l'arborescence « année / mois » aux noms opaques. Colonnes `files.categorie` et `files.ged_nom` (migration `0073`).
- **Reclassement du stockage existant** : depuis **Paramétrages › GED**, un bouton **« Reclasser maintenant »** (précédé d'une **simulation**) range les documents déjà déposés : chaque nœud est **déplacé** (l'identifiant ne change pas, donc les liens restent valides), **renommé** et complété de ses métadonnées, **sans re-téléversement**. **Rejouable** : un document déjà classé est ignoré. Aussi disponible en ligne de commande (`scripts/reclasser-ged.js`) et via `POST /ged/stockage/reclassement`. (`reclassement.js`, adaptateurs Alfresco et simulateur : `deplacer`, `majNode`, `nodeInfo`.)
- **Qualité** : correction des deux erreurs de lint préexistantes (`annexes.service.js` — caractère de contrôle ; `annexes.routes.js` — échappement inutile) et nettoyage d'une variable de test inutilisée (`lot21-president-maire.test.js`).
...
- **Élus — données DSIHUB enrichies** : la synchronisation reprend désormais le **sexe** (« M » / « F »), la **liste / groupe politique** (« IVRY AVANT TOUT »…) et les **délégations fonctionnelles** de chaque élu (migration `0070`) ; le rattachement aux groupes politiques s'appuie sur la liste du Hub, et les délégations sont conservées pour les usages à venir (affichage dans **Paramétrages › Membres**).
...
- **Fin de circuit différente** : pour une décision ou un arrêté, le circuit reste le même que pour les délibérations, mais à la fin le dossier passe à l'état **« À signer »** (au lieu d'être inscrit au conseil) et part en **signature du maire**. Le circuit peut être spécialisé par type d'acte dans **Paramétrages › Circuits**. (`engine.js` : statuts `a_signer`, `signe`, `signature_refusee`, migration `0061`.)
- **Délibérations d'autorisation** : une décision est prise par le maire dans le cadre d'une délégation du conseil. Un bloc **« Délibérations d'autorisation »** permet de **lier les délibérations adoptées** qui l'autorisent (recherche dans la bibliothèque) ; **la complétude exige au moins une autorisation** avant l'envoi. Table `acte_liens` (migration `0061`).
- **Configuration du parapheur** (Paramétrages › **Parapheur (signature)**) : choix du **parapheur** — **DSIHUB par défaut** (module parapheur du Hub DSI, appelé en machine-à-machine), **iParapheur** prévu mais **non implémenté** —, **actif/inactif**, **mode dev** (tous les envois vers une **adresse d'essai unique**) ou **prod** (au **signataire** nommé), **compte technique** du Hub (mot de passe **chiffré au repos**, jamais renvoyé), **signataire** et **test de connexion**. (`parapheur.service.js`, `adapters/parapheur-dsihub.js`, `parapheur-simulateur.js`.)
- **Pastille de type** : dans les listes et sur la fiche, l'**icône du type** (délibération, décision, arrêté) s'affiche **seule**, à côté du titre, et le **libellé reste** en infobulle.
- **Une décision, juste une décision** : une décision n'a **ni séance à viser**, **ni élu rapporteur**, **ni exposé des motifs**, **ni visas** — seulement l'acte lui-même. Les **articles** du dispositif s'enchaînent **sans retour à la ligne**. (`referentiels/seeds.js`, `textes.service.js`, migration `0062`, `typeset.js`.)
- **Mode de signature et envoi maîtrisé** : le **mode de signature** se choisit (**P12** — certificat personnel — **par défaut**, **signature manuscrite** ou **SMS** avec le **portable** du signataire) ; le **document remis au parapheur porte son vrai titre** (jamais « blob ») et les **métadonnées de l'acte** (rubrique, direction, rédacteur, séance…) lisibles dans le parapheur ; la **position de signature** est transmise au parapheur (sans elle, le Hub n'affiche pas le document : « Aucune position définie ») ; un envoi **en erreur** peut être **renvoyé** ; un bouton **« Envoyer un document de test »** vérifie la chaîne de bout en bout ; le bouton d'envoi se met **en surbrillance** quand l'envoi est impossible ; un **administrateur ou le SCC** peut envoyer une décision en signature **sans attendre la fin du circuit**. (migration `0063`, `parapheur.service.js`, `parapheur.routes.js`, `adapters/parapheur-dsihub.js`, `typeset.js`.)
- **Journal des échanges** : ce qui est **envoyé** au parapheur et ce qui est **retourné** (demande, accusé, état, retour) est **journalisé** et consultable depuis la **fiche du dossier** (bloc Signature) et l'**administration**. Sans Hub configuré, un **simulateur** permet de dérouler la signature (retour **signé** ou **refusé**). (`parapheur_journal`, `parapheur_envois`.)

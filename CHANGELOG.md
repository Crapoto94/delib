...
- **Fiche du dossier** : le **montant** d'un impact budgétaire est explicitement **facultatif** ; l'**ajout ou le retrait d'une commission** ne réinitialise plus les **saisies en cours** ; choix de la **délégation de l'élu rapporteur** quand il en porte plusieurs (migration `0072`), reportée dans les listes et le **tri de l'ordre du jour**. La liste des rapporteurs n'affiche que les **noms et prénoms** (fonction et délégations masquées — la délégation se choisit au champ suivant) ; le **nom de l'élu** est en **gras**.
- **Pièces jointes à chaque étape** : un dossier accepte des **annexes à toutes les étapes du circuit**, et plus seulement pendant la rédaction. Le **détenteur de l'étape courante** (ou son délégué habilité) et le rédacteur peuvent **ajouter, remplacer ou retirer** une pièce. Nouveau droit `canAttach` (`acl.js`, crochet du moteur `engine.js`) appliqué aux mutations d'annexes (`annexes.service.js`).

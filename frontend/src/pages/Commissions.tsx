    ...
    <div>
      ...
      {list.loading ? <Loading /> : !list.data?.length ? <div className="card"><Empty>Aucune commission. Créez-en une pour pouvoir la rattacher aux actes.</Empty></div> : (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{list.data.map((c) => (
          <button key={c.id} onClick={() => setOpen(c.id)} className="card p-5 text-left hover:shadow-lift"><h3>{c.nom}</h3><p className="mt-1 text-mute">{c.description || '—'}</p><p className="mt-3"><Badge tone="blue">{c.nbMembres} membre(s)</Badge> {!c.actif && <Badge>inactive</Badge>}</p></button>))}</div>)}

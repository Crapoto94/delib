    ...
    <div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <ol>{order.map((it) => {
              ...
              return (
                <li key={it.id} draggable={canEdit && !retire} onDragStart={() => setDragFrom(idx)} onDragEnd={() => { setDragFrom(null); setOver(null); }} onDragOver={(e) => { if (!retire) { e.preventDefault(); setOver(idx); } }}
                  ...
                  className={`flex items-center gap-3 border-b border-line px-3 py-3 ${retire ? 'bg-slate-50 opacity-60' : 'bg-white'} ${dragFrom === idx ? 'opacity-40' : ''} ${over === idx && dragFrom !== idx ? 'shadow-lift ring-2 ring-action' : ''}`}>
        ...
        <aside className="card h-fit" aria-label="Dossiers en attente d'affectation">
          <div className="border-b border-line px-4 py-3"><h3>En attente d'affectation <Badge tone="blue">{pendingLoad.data?.length ?? 0}</Badge></h3><p className="text-[12px] text-mute">Dossiers dont le circuit est terminé.</p></div>

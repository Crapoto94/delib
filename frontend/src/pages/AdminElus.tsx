...
function SmsPasserelle({ o }: { o: number }) {
  ...
  const sim = f.mode !== 'http'; const apm = f.mode === 'apm';
  return (
    <section className="card space-y-3 p-5"><h3>Passerelle SMS (mot de passe oublié)</h3>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <Field label="Mode"><Select className="input" value={f.mode} onChange={(e) => setF({ ...f, mode: e.target.value })}><option value="simulation">Simulation (aucun SMS n’est envoyé)</option><option value="apm" disabled={d.data.config.apmDisponible === false}>API de la Ville — APM (clé déjà configurée){d.data.config.apmDisponible === false ? ' — non configurée sur ce serveur' : ''}</option><option value="http">Passerelle propre (HTTP) — paramétrage spécifique</option></Select></Field>
        {apm && <p className="rounded bg-soft px-3 py-2 text-[13px] md:col-span-2">Les SMS partent par l’<b>API de la Ville</b> (<code>api.ivry.local</code>, <code>POST /api/v1/sms/send</code>) avec la clé APM déjà configurée sur le serveur : <b>rien à saisir</b>. La clé doit avoir la permission <code>sms_send</code>.</p>}

  ...
  const svc = {
    ...
    async documentsSeance(ctx, organismeId, seanceId) {
      ...
      for (const x of await db.all("SELECT * FROM tlt_transactions WHERE seance_id = $1 AND etat = 'poste' AND ar_id IS NOT NULL", [seanceId])) {
        if (x.ar_file_id) add({ key: `t${x.id}:ar-xml`, acteId: x.acte_id, dossier: sous('legalite'), nom: sur(`${d}_ARActe_${x.numero_transmis}.xml`), mime: 'application/xml', empreinte: sha({ ar: x.ar_id, f: x.ar_file_id }), produire: async () => (await tlt.arXml(ctx, org, x.id)).buffer, description: `Accusé de réception de la préfecture (ARActe XML) — ${x.numero_transmis}` });
        add({ key: `t${x.id}:extrait-ar`, acteId: x.acte_id, dossier: sous('legalite'), nom: sur(`${d}_extrait-du-registre-tamponne_${x.numero_transmis}.pdf`), empreinte: sha({ ar: x.ar_id, aff: x.date_affichage ?? null, tok: t ? t.version : 0 }), produire: async () => (await tlt.extrait(ctx, org, x.id)).buffer, description: `Extrait du registre tamponné (AR de la préfecture) — ${x.numero_transmis}` });
